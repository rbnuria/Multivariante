...
\begin{document}
	...
	Problema: saber que es $nq$ y no $qn$, podemos solucionarlo con notación $Vec_{n,q}$
	...
	\begin{exercise}
		...
		\begin{enumerate}[a)]
			...
			Por otro lado, $$Vec(X^t)^t Vec(Y) = \sum_{i = 1}^{n} x_{:,i} y_{i,:} = \sum_{j=1}^{m} \sum_{i = 1}^{n} x_{ji} y_{ij}  = \sum_{j=1} \sum_{i=1} x_{ij}^ty_{ij}$$
	...
	$$ A \otimes B \otimes C = K_{ms,p}(C \otimes A \otimes B)K_{q,nt} = K_{m,sp}(B \otimes C \otimes A)K_{tq,n}$$ Así, podemos enunciar el siguiente teorema en el que se muestran algunas propiedades:
	...
	Sea X una matriz simétrica de orden $n\times n$, entonces $X= \displaystyle \sum_{i=1}^{n} \sum_{j=1}^{n}x_{ij}[J_{ij}+J_{ji}(1-\delta_{ij})]$.
	
	Por tanto, $Vec(X) = \displaystyle \sum_{i=1}^{n} \sum_{j=1}^{n} x_{ij}[e_{(j-1)n+i}+e_{(i-1)n+j}(1-\delta_{ij})]$ y las columnas de la matriz de transición son $e_{(j-1)n+i}+e_{(i-1)n+j}(1-\delta_{ij})$ para $i=1,...,n$, $j=i,...,n$. Es de dimensión $n^2 \times \displaystyle \frac{n(n+1)}{2}$.
	...
	\textit{Demostración:}
	\begin{enumerate}
		\item Sea $X_{n\times n}$ diagonal, entonces $X=\sum_{i=1}^{n}x_{ii}J_{ii}$. Por tanto, $Vec(X) = \sum_{i=1}^{n}x_{ii}e_{(i-1)n+i}$ y las columnas de la matriz de transición son $e_{(i-1)n+i}$, $i=1,...,n$. Es de dimensión $n^2 \times n$
		\item Sea $X_{n\times n}$ triangular superior, entonces \\$X=\sum_{i=1}^{n} \sum_{j=i}^{n}x_{ij}J_{ij}$. Por tanto, $Vec(X) = \sum_{i=1}^{n} \sum_{j=i}^{n}x_{ij}e_{(j-1)n+i}$ y las columnas de la matriz de transición son $e_{(j-1)n+i}$, $i=1,...,n$, $j=i,...,n$. Es de dimensión $n^2 \times \frac{n(n+1)}{2}$
		\item Sea $X_{n\times n}$ triangular inferior, entonces \\$X=\sum_{i=1}^{n} \sum_{j=1}^{i}x_{ij}J_{ij}$. Por tanto, $Vec(X) = \sum_{i=1}^{n} \sum_{j=1}^{i}x_{ij}e_{(j-1)n+i}$ y las columnas de la matriz de transición son $e_{(j-1)n+i}$, $i=1,...,n$, $j=1,...,i$. Es de dimensión $n^2 \times \frac{n(n+1)}{2}$
		\item Sea $X_{n\times n}$ antisimétrica (en $\mathbb{R}$ los valores de la diagonal son 0), entonces \\$X=\sum_{i=1}^{n-1} \sum_{j=i+1}^{n}x_{ij}[J_{ij}-J_{ji}]$. Por tanto, $Vec(X) = \sum_{i=1}^{n-1} \sum_{j=i+1}^{n}x_{ij}[e_{(j-1)n+i}-e_{(i-1)n+j}]$ y las columnas de la matriz de transición son $e_{(j-1)n+i}-e_{(i-1)n+j}$, $i=1,...,n-1$, $j=i+1,...,n$. Es de dimensión $n^2 \times \frac{n(n-1)}{2}$
	\end{enumerate}
	\qed
	...
	Notemos que si f es diferenciable en c, entonces f es continua en c.
	...
	Al igual que en el caso anterior tenemos el siguiente teorema que nos ayuda a identificar los jacobianos de funciones matriciales.
	...
	En el siguiente teorema se recogen algunas propiedades útiles de las diferenciales matriciales y de las matrices jacobianas:
	...
	Veamos un ejemplo de cálculo de matrices jacobianas:
	
	\begin{example}
		Sea $F_{m \times p}$ una función diferenciable y consideremos $G(X)=AF(X)B$ con $A_{n \times m}$ y $B_{p\times q}$ matrices constantes. Entonces, siguiendo el procedimiento que acabamos de explicar y aplicando el teorema anterior, se tiene $dG(X)=AdF(X)B$, de donde $dVec(G(X)) = (B^t \otimes A)dVec(F(X)) = (B^t \otimes A)DF(X)dVec(X)$ y, por lo tanto, concluimos que $DG(X)=(B^t \otimes A)DF(X)$.
	\end{example}
	...
	No existe una única definición para la derivada de una función de argumento matricial y esto supone un problema a la hora de usar el cálculo diferencial matricial. Son muchas las definiciones clásicas que existen, pero nosotros nos quedaremos con las de Mac Rae y Dwyer que generalizan a las demás y que están relacionadas como veremos también.
	...
	Puede ocurrir que estas definiciones no conduzcan a la matriz jacobiana definida previamente. Así surge la siguiente definición en la que precisamente se emplea la misma notación que la matriz jacobiana debido a su coincidencia.
	
	\begin{definition}
		(Definición de derivada matricial según Magnus y Neudecker)\\
		Sea $F:\mathbb{M}_{n\times q}\rightarrow \mathbb{M}_{m\times p}$. Se define la derivada de $F$ respecto de $X\in \mathbb{M}_{n\times q}$ como la matriz $mp\times nq$ dada por \textbf{$DF(X)=\displaystyle\frac{\partial Vec(F(x))}{\partial Vec^t(x)}$}
	\end{definition}
	...
	\begin{theorem}
		...
		\begin{enumerate}
			\item $\displaystyle\frac{\partial F(x)}{\partial x^t} =(\frac{\partial (F(x))^t}{\partial x})^t $
			
			\item $\displaystyle \frac{\partial (F(x))^t}{\partial x^t} =(\frac{\partial F(x)}{\partial x})^t$
		\end{enumerate}
	\end{theorem}
	
	Como consecuencia obtenemos el siguiente corolario que engloba la derivada matricial de múltiples funciones matriciales.
	...
	Cabe preguntarse también qué ocurre con las derivadas de dos funciones importantes como son la traza y el determinante.
	...
	De estos teoremas se pueden deducir numerosas propiedades.
	
	\subsubsection*{Derivadas matriciales asociadas a determinantes}
	
	El determinante es una función muy empleada en Estadística ya que forma parte de la verosimilitud asociada a matrices aleatorias, así que veamos qué ocurre con su derivada.
	...
	Veamos también la expresión de la diferencial y la derivada matricial del logaritmo de un determinante.
	
	\begin{corollary}
		Sea S un abierto de $\mathbb{M}_{n\times q}$ y $F: S \rightarrow \mathbb{M}_{p \times p}$ tal que F es diferenciable sobre S. Entonces, apra los puntos X en los que |F(X)|>0, la función $log|F|:S\rightarrow \mathbb{R}$ dada por (log|F|)(X) = log|F(X)| es diferenciable. Además $dlog(|F|(X)) = tr[F^{-1}(X)dF(X)]$, $Dlog(|F|(X)) = Vec^t(F^{-1'}(X))DF(X)$ y 
		
		$$\frac{\partial log(|F|(X))}{\partial X} = F^{-1'}(X) * \frac{\partial F(X)}{\partial X} = \sum_{i=1}^{n} \sum_{j=1}^{q}\left(F^{-1'}(X)* \frac{\partial F(X)}{\partial x_{ij}} \right)J_{ij}$$
		$$= \sum_{i=1}^{n} \sum_{j=1}^{q} tr\left[F^{-1}(X)\frac{\partial F(X)}{\partial x_{ij}}\right]J_{ij}$$
	\end{corollary}
	...
	\begin{definition}
		...
		En particular, si $B_1, ..., B_m$ son matrices cuadradas y tenemos la matriz $B = \left({\begin{array}{c}
			...
			\vdots \\
			B_m
			\end{array} } \right)$ entonces $B' = [B_1^t, ..., B_m^t]$ y con ello tenemos que	$ B'_v = \left({\begin{array}{c}
			B'_1\\
			B'_2\\
			\vdots
			\\
	...
	$-\frac{1}{2}\frac{\partial tr[\Sigma^{-1}A]}{\partial \Sigma}=-\frac{1}{2}\frac{\partial tr[A\Sigma^{-1}]}{\partial \Sigma}  = \frac{1}{2}(\Sigma^{-1}A\Sigma^{-1})^t.$
	
	Ahora, atendiendo a la derivada del logaritmo de un determinante, tenemos que:
	
	$\frac{-N}{2} \frac{\partial log(|\Sigma|)}{\partial \Sigma} = \frac{-N}{2}(\Sigma^{-1})^t*\frac{\partial \Sigma}{\partial \Sigma} = \frac{-N}{2}(\Sigma^{-1})^t * Vec(I_n)Vec(I_n^t) = \frac{-N}{2}(\Sigma^{-1})^t$
	
	Uniendo tenemos que:
	
	$$\frac{\partial F(\Sigma)}{\partial \Sigma} = -\frac{N}{2}(\Sigma^{-1})^t + \frac{1}{2}(\Sigma^{-1}A\Sigma^{-1})^t = -\frac{N}{2}(\Sigma^{-1}) + \frac{1}{2}(\Sigma^{-1}A^t\Sigma^{-1}) = -\frac{N}{2}(\Sigma^{-1}) + \frac{1}{2}(\Sigma^{-1}A\Sigma^{-1}) $$
	
	$$-\frac{N}{2}(\Sigma^{-1}) + \frac{1}{2}(\Sigma^{-1}A\Sigma^{-1}) = 0 \Leftrightarrow N(\Sigma^{-1})=(\Sigma^{-1}A\Sigma^{-1})\Leftrightarrow N=\Sigma^{-1}A \Leftrightarrow \Sigma = \frac{1}{N}A$$
	
	Por tanto $\hat{\Sigma} = \frac{1}{N}A$ .
